...
export const Voice: React.FC<VoiceProps> = ({ setActiveTab }) => {
  ...
  const handleProcessVoice = async () => {
    ...
    // formData.append('audio', audioBlob); // Need the actual Blob, not just the URL
    // try {
    //   // const response = await fetch('/api/clone-voice', { method: 'POST', body: formData });
    //   // const result = await response.json();
    //   // Handle success - maybe store clone ID or confirmation
    await new Promise((resolve) => setTimeout(resolve, 2000)); // Simulate processing
    alert("Voice sample processed successfully! (Simulated)");
    //   setActiveTab('some-next-step'); // Navigate after successful cloning
    // } catch (error) {
